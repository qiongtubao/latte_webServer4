			...
			, Querystring = require("querystring");
				...
				(function() {
					...
					this.request = function(pathname, req, res, next) {
						...
						//console.log(pathname, self.all);
						
						if(self.all[pathname]) {
							res.handleType = "io";
	 						req.gets = Querystring.parse(url.query);
	 						self.all[pathname].handleRequest(req, res);
	 						return ;
	 					}else{
	 						next();
	 					}
					...
					this.handleUpgrade = function(req, socket, head) {
						...
						if(self.all[pathname]) {
							req.gets = Querystring.parse(url.query);
							self.all[pathname].handleUpgrade(req, socket, head);
						}
					}
				}).call(Ios.prototype);
				module.exports = Ios;
